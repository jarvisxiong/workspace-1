 ...
 */
@Component("PO_Filter")
public class PO_Filter implements RegionFilter {

    static final Logger logger = LogManager.getLogger(PO_Filter.class.getName());
...
//        
    ...
    public boolean doFilterForRegion(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
        ...
        try {
            ...
            logger.info("---PO FILTER CALLED ---- {}", domain.getDefaultUrl());
            MobileClub club = UmeTempCmsCache.mobileClubMap.get(domain.getUnique());

            logger.info("---PO FILTER CALLED ---- {} Club {}", domain.getDefaultUrl(), club.getName());
            ...
            logger.info("PO FILTER PATH RECEIVED IS " + path);
            if (path.contains("jpt.jsp")) {
                path = "index_main.jsp";
            }
            //==Download Link 
...
//System.out.println("INSIDE forwardRequest : value of defaultServlet "+defaultServlet +" path: "+path+" service type: "+service.getServiceType());
            ...
            //System.out.println("PO_Filter Forwarding to /"+path);
            umesdc.getCxt().getRequestDispatcher("/" + path).forward(req, res);
            return;
        } catch (Exception e) {
            if (req != null) {
                System.out.println("PO_FILTER.forwardRequest exception req.getServerName(): " + req.getServerName());
                System.out.println("PO_FILTER.forwardRequest exception domain: " + domain);
                System.out.println("PO_FILTER.forwardRequest exception service: " + service);
                System.out.println("PO_FILTER.forwardRequest exception user: " + user);
                System.out.println("PO_FILTER.forwardRequest exception lang: " + lang);
                System.out.println("PO_FILTER.forwardRequest exception serviceType: " + serviceType);
                System.out.println("PO_FILTER.forwardRequest exception path: " + path);
            }

            System.out.println("PO_Filter UmeCore.forwardRequest exception : " + e.getMessage());
                //e.printStackTrace();
    ...
    public void evaluateIPAddress(HttpServletRequest req) {
        ...
        logger.info("---IP ADDRESS IN PO FILTER  ---- {}", ipAddress);
