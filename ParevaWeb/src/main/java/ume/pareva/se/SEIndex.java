 ...
 * @author madan
 */
@WebServlet(name = "SEIndex", urlPatterns = {"/SEIndex"})
public class SEIndex extends HttpServlet {
     private final Logger logger = LogManager.getLogger(SEIndex.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        logger.info("seindex "+"ProcessREQEST is called upon ");

        HttpSession session = request.getSession();
        logger.info("seindex "+"session id is  "+session.getId());
        ServletContext application = request.getServletContext(); //.setAttribute(null, request);
        logger.info("seindex "+"servletContext is  "+application.getContextPath());
        ...
        logger.info("seindex "+" after SDC request ");
        ...
        System.out.println("seindexindex "+" FileName is  "+fileName);
        ...
        logger.info("seindex "+"contentUrl is  "+cloudfronturl);
        ...
        if (!enMsisdn.equals("")) {
            ...
            if (!deMsisdn.equals("")) {
                if (deMsisdn.startsWith("0")) {
                    deMsisdn = "46" + deMsisdn.substring(1);
                }
                msisdn = deMsisdn;
            }
        }
        ...
        try {
            ...
            networkname = mobilenetwork.getMobileNetwork(club.getRegion().toUpperCase(), networkname);
                ...
                System.out.println("seindex INDEX JSP LOGGING TO CAMPAIGN LOG ");
                if (!aReq.get("cid").equalsIgnoreCase("") && session.getAttribute("bilogged")==null) {
                    campaigndao.log("seindex", landingPage, msisdn, msisdn, handset, domain, campaignId, clubUnique, "INDEX", 0, request, response, networkname,"","","",pubId);
            ...
            System.out.println("seindex Index Error for campaignlog " + e);
            logger.error("seindex INDEX ERROR FOR CAMPAIGN LOG {}", e.getMessage());
            logger.error("seindex INDEX ERROR FOR CAMPAIGN LOG ", e);
        }
        ...
        if(club!=null){
            ...
            return;
                ...
                logger.error("seindex EXCEPTION {}", e.getMessage());
                logger.error("seindex EXCEPTION ", e);
            } 
