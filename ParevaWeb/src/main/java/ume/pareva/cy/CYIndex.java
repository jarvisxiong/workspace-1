 ...
 * @author madan
 */
@WebServlet(name = "CYIndex", urlPatterns = {"/CYIndex"})
public class CYIndex extends HttpServlet {
	private final Logger logger = LogManager.getLogger(CYIndex.class.getName());
	 ...
	 */
	 ...
	 protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		 ...
		 logger.info("cyindex "+"ProcessREQEST is called upon ");

		 HttpSession session = request.getSession();
		 logger.info("cyindex "+"session id is  "+session.getId());
		 ServletContext application = request.getServletContext(); //.setAttribute(null, request);
		 logger.info("cyindex "+"servletContext is  "+application.getContextPath());
		 ...
		 logger.info("cyindex "+" after SDC request ");
		 ...
		 logger.info("cyindex "+"contentUrl is  "+cloudfronturl);
		 ...
		 if (!enMsisdn.equals("")) {
			 ...
			 if (!deMsisdn.equals("")) {
				 if (deMsisdn.startsWith("0")) {
					 deMsisdn = "357" + deMsisdn.substring(1);
		 ...
		 try {
			 ...
			 System.out.println("cyindex JSP LOGGING TO CAMPAIGN LOG ");
			 if (!aReq.get("cid").equalsIgnoreCase("") && session.getAttribute("bilogged")==null) {
				 campaigndao.log("cyindex", landingPage, msisdn, msisdn, handset, domain, campaignId, clubUnique, "INDEX", 0, request, response, networkname,"","","",pubId);

				 if(msisdn!=null && !msisdn.equalsIgnoreCase(""))
					 campaigndao.log("cyindex", landingPage, msisdn, msisdn, handset, domain, campaignId, clubUnique, "IDENTIFIED", 0, request, response, networkname,"","","",pubId);
			 ...
			 System.out.println("cyindex Error for campaignlog " + e);
			 logger.error("cyindex ERROR FOR CAMPAIGN LOG {}", e.getMessage());
			 logger.error("cyindex ERROR FOR CAMPAIGN LOG ", e);
		 }
		 ...
		 if(club!=null){
			 ...
			 System.out.println("cyindex step1 "+transactionid+" msisdnrouter: "+msisdnrouter);
			 ...
			 //System.out.println("cyindex step1 URL"+msisdnrouter);
			 ...
			 if(userclubdetails.getBillingType().equalsIgnoreCase("subscription")){
				 ...
				 if(userclubdetails.getServiceType().equalsIgnoreCase("content")) {
					 ...
					 try {
						 ...
						 logger.error("cyindex EXCEPTION {}", e.getMessage());
						 logger.error("cyindex EXCEPTION ", e);
					 } 
				 ...
				 System.out.println("cyindex realgirls  adhoc service condition ");
				 if(user!=null){
					 request.setAttribute("user", user);
					 SdcMobileClubUser clubUser=umemobileclubuserdao.getClubUserByMsisdn(user.getParsedMobile(),club.getUnique());
					 System.out.println("cyindex user is not null ");
					 if(clubUser!=null && clubUser.getActive()==1){
						 System.out.println("cyindex clubUser is active "+clubUser.getParsedMobile());
						 ...
						 if(billingplan!=null && billingplan.getActiveForBilling()<0 && billingplan.getExternalId().equalsIgnoreCase("adhocpending")){
							 ...
							 campaigndao.log("cyadhoc", clubUser.getLandingpage(), clubUser.getParsedMobile(),clubUser.getParsedMobile(), handset,domain, clubUser.getCampaign(),clubUser.getClubUnique(), "SUBSCRIBED", 0, request,response,clubUser.getNetworkCode().toLowerCase(),"","","",pubId);
							 try {
