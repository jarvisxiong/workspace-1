...
@WebServlet("/AEUnsubscribe")
public class AEUnsubscribe extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getLogger(AEUnsubscribe.class.getName());
       
	@Autowired
	TemplateEngine templateEngine;//
	
    public AEUnsubscribe() {
        super();
    }
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			frEngine.getTemplate("unsubscribe").evaluate(writer, context);
		}catch(Exception e){
    		logger.error("Error Rendering Unsubscribe Template");
    		e.printStackTrace();
    	}
