 ...
 * @author trung
 */
@WebServlet(name = "AEConfirm", urlPatterns = {"/AEConfirm"})
public class AEConfirm extends HttpServlet {

    private static final long serialVersionUID = 1L;
    private static final Logger logger = LogManager.getLogger(AEConfirm.class.getName());
    ...
    public void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        System.out.println("*****************************************AEConfirm Parameter Start***********************************************");
        ...
        System.out.println("*****************************************AEConfirm Parameter End*************************************************");

        //String network = getUaeNetwork(callbackMap.get("opid"));
        String network = "etisalat";
        ...
        String campaignId = (String)session.getAttribute("cid");      
        if(campaignId==null)
            campaignId = "";
        
        String msisdn = callbackMap.get("msisdn");
        if(msisdn!=null && msisdn.startsWith("971")){
            ...
            System.out.println("subResponse AEConfirm: " + subResponse + " - msisdn: " + msisdn);
            ...
            if ((subResponse.equals("OLD SUBSCRIPTION RECORD UPDATED SUCCESSFULLY") || subResponse.equals("SUBSCRIPTION RECORD CREATED SUCCESSFULLY"))) {                    
                ...
                this.redirectUserAndSaveSession(callbackMap, request, response);
            } 
        }
        /*
...
//                response.sendRedirect("http://" + dmn.getDefaultUrl());
            ...
            } else if (callbackMap.get("action").equalsIgnoreCase("SUB")) {
//                String campaignId = getCampaignAndNetworkType(callbackMap).get("campaign");
    ...
    private void sendWelcomeSms(Map<String, String> callbackMap, HttpServletRequest request, MobileClub club) {
        ...
        if(welcomeMessages!=null && !welcomeMessages.isEmpty()){
            System.out.println("Send Welcome SMS AE Club Details: " + msisdn);
...
//        String landingPage = callbackMap.get("opt2");
    ...
    private void redirectUserAndSaveSession(Map<String, String> callbackMap, 
        ...
        HttpSession session = request.getSession();
        try {
            UmeUser user = umeuserdao.getUser(callbackMap.get("msisdn"));
            response.sendRedirect("http://" + dmn.getRedirectUrl() + "/?id=" + user.getWapId());
        } catch (Exception e) {
