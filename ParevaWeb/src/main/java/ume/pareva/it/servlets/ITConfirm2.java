 ...
 * @author madan
 */
@WebServlet(name = "ITConfirm2", urlPatterns = {"/ITConfirm2"})
public class ITConfirm2 extends HttpServlet {
    ...
    private final Logger logger = LogManager.getLogger(ITConfirm2.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        logger.info("itconfirm " + "ProcessREQEST is called upon ");

        HttpSession session = request.getSession();
        logger.info("itconfirm " + "session id is  " + session.getId());
        ServletContext application = request.getServletContext(); //.setAttribute(null, request);
        logger.info("itconfirm " + "servletContext is  " + application.getContextPath());
...
//        boolean validMsisdn = true; //validationutil.isNORValidPhone(msisdn);
        ...
        System.out.println("itrconfirm VALIDATION of msisdn " + msisdn + " is " + validMsisdn);
        if (validMsisdn) {
            campaigndao.log("itrconfirmweb", confirmlanding, msisdn, msisdn, handset, domain, campaignId, club.getUnique(), "MANUAL", 0, request, response, networkname.toLowerCase(), "", "", "", pubId);
            System.out.println("itrconfirm VALIDATION of msisdn INSIDE CONDITION " + msisdn + " is " + validMsisdn);
            ...
            System.out.println("itrconfirm the value of sendTeaser is " + sendTeaser + "For msisdn " + msisdn);
            if (sendTeaser) {
                ...
                System.out.println("itr compmessage ie confirm " + toAddress[0] + " msg is " + msg + " shortCode is " + shortCode);
                compmessage.requestITSendSmsTimer(toAddress, msg, shortCode, club, userClubDetails, 20 * 10, false);
            }
