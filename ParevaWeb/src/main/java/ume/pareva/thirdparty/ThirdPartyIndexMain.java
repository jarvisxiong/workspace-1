 ...
 * @author madan
 */
public class ThirdPartyIndexMain extends HttpServlet {
        ...
        if (!item.equals("")) {
            ...
            if (!itype.equals("")) {
                try {
                    ...
                    System.out.println("ZA IndexMain Exception line no. 159 "+e); e.printStackTrace();
                    return;
                }
            }
        }
...
//                rd = this.getServletContext().getRequestDispatcher("/" + dmn.getRedirectUrl()+ "/?m="+Misc.encrypt(user.getParsedMobile()));
//                rd.include((ServletRequest)request, (ServletResponse)response);
