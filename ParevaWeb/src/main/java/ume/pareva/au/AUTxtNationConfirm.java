 ...
 * @author Madan
 */
@WebServlet(name = "AUTxtNationConfirm", urlPatterns = {"/AUTxtNationConfirm"})
public class AUTxtNationConfirm extends HttpServlet {
    ...
    UmeClubDetailsDao umeclubdetailsdao;
     ...
     private final Logger logger = LogManager.getLogger(AUTxtNationConfirm.class.getName());
...
protected void processRequest(HttpServletRequest request, HttpServletResponse response)
     ...
     ThreadContext.put("ROUTINGKEY", "AU");
        logger.info("AUconfirm "+"ProcessREQEST is called upon ");

        HttpSession session = request.getSession();
        logger.info("AUconfirm "+"session id is  "+session.getId());
        ServletContext application = request.getServletContext(); //.setAttribute(null, request);
        logger.info("AUconfirm "+"servletContext is  "+application.getContextPath());
	...
	userClubDetails=UmeTempCmsCache.umeClubDetailsMap.get(club.getUnique());
        ...
        //if(msisdn.startsWith("0061")) msisdn=msisdn.substring(2);
        ...
        System.out.println("AUconfirm VALIDATION of msisdn "+msisdn+" is "+validMsisdn);
        if(validMsisdn){
            
        if(session.getAttribute("msisdnidentified")==null){
         campaigndao.log("AUconfirmweb", confirmlanding, msisdn,msisdn, handset,domain, campaignId, club.getUnique(), "MANUAL", 0, request,response,networkname.toLowerCase(),"","","",pubId);   
        }
        System.out.println("AUconfirm VALIDATION of msisdn INSIDE CONDITION "+msisdn+" is "+validMsisdn);
	...
	if (!userUnique.equals("")) user = umeuserdao.getUser(msisdn);
        ...
        //==========END IF USER EXISTS OR NOT ============================
              ...
              System.out.println("AUconfirm the value of sendTeaser is "+sendTeaser+ "For msisdn "+msisdn) ;
              if(sendTeaser){
                  ...
                  System.out.println("AU compmessage ie confirm "+toAddress[0]+" msg is "+msg+" shortCode is "+shortCode);
