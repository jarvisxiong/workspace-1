...
@Component("AU_Filter")
public class AU_Filter implements RegionFilter{
...
//        
    ...
    public boolean doFilterForRegion(HttpServletRequest  req, HttpServletResponse res) throws IOException, ServletException {
        ...
        //System.ou t.println("---ZA FILTER CALLED ---- ");
        ...
        try{
            ...
            System.out.println("AU DEBUG ---AU FILTER CALLED ---- "+domain.getDefaultUrl());
            MobileClub club = UmeTempCmsCache.mobileClubMap.get(domain.getUnique());
            System.out.println("AU DEBUG ---AU FILTER CALLED ---- "+domain.getDefaultUrl()+" Club "+club.getName());
            
            user = userauthentication.authenticateUser(domain, req, res);
            System.out.println("AU DEBUG ---AU FILTER AFTER USED ID " + (user==null?"null":user.getParsedMobile()));
            
            
             if ((SdcRequest.get("anyx_kplg", req).equals("true") || !SdcRequest.get("id", req).equals("")) && user!=null && !user.getActiveClubCode().equals("")) {
                // Syste m.out.println("Setting ANYX_KPLG Cookie: " + user.getActiveClubCode());
            ...
            System.out.println("AU DEBUG AU FILTER======== PATH RECEIVED IS "+path);
            ...
            //==Download Link 
               if (!get("axud", req).equals("") || !get("anyx_usedef", req).equals("")) 
               {
                ...
                System.out.println("AU DEBUG AU FILTER SERVICE PATH::>> DEFAULTSERVLET : "+defaultServlet +" path: "+path);
            }
               
            else defaultServlet = false;
               
                if (path.startsWith("/")) { 
                path = path.substring(1); System.out.println("AU DEBUG AU FILTER PATH WITH / "+path); 
                ...
                if (user==null) service = umesdc.getServiceMap().get(domain.getUnique() + "_pub");
                    ...
                    System.out.println("AU DEBUG AU FILTER ===  SERVICE in path "+path +" domain unique pri "+domain.getUnique()+"_pri"+" Services in "+umesdc.getServiceMap().get(domain.getUnique() + "_pri"));
                    service = umesdc.getServiceMap().get(domain.getUnique() + "_pri");
                }
                path = service.getDefaultPage();
                System.out.println("AU DEBUG AU FILTER ===  PATH DEFAULT PAGE "+path);
            }
                ...
                if ((i = path.indexOf("/"))>-1) {
                    ...
                    if (user==null) {
                        ...
                        //Syst em.out.println("SERVICE IN UmeFilter:: "+domain.getUnique()+"_pri" +" services "+umesdc.getServiceMap().get(domain.getUnique() + "_pri"));
                        service = umesdc.getServiceMap().get(domain.getUnique() + "_pri");
                    }
                }

                //Syst em.out.println("SERVICE: " + service);
                //System.ou t.println("Type: " + serviceType);
            }
      if(user==null){
                    String addparam="";
                    ...
                    if (campaignId!=null && !campaignId.equals("")) {
                        System.out.println("AU DEBUG AU FILTER === " + campaignId);
            ...
            System.out.println("AU DEBUG AU FILTER SERVICE: " + service);
            System.out.println("AU DEBUG AU FILTER AU_FILTERSERVICETYPE: " + service.getServiceType());
            System.out.println("AU DEBUG AU FILTER AU_Filter service "+service.getDirectory() + ": " + service.getUnique());
            System.out.println("AU DEBUG AU FILTER AU_Filter Type: " + serviceType);
            System.out.println("AU DEBUG AU FILTER AU_Filter PATH: " + path);
                
            System.out.println("AU DEBUG AU FILTER USER AUTHENTICATION ==== "+userauthentication.authenticateService(service, user, domain));
...
//            }
            ...
            //System.out.println("AU FILTER REDIRECTION CALLED UPON :"+serviceType+ " path: "+path+"  service : "+service.getUnique());
    ...
    private void forwardRequest(ServiceType serviceType, String path, UmeDomain domain, SdcService service, UmeUser user, SdcLanguage lang,
        ...
        System.out.println("AU FILTER INSIDE AUDEBUG forwardRequest : value of defaultServlet "+defaultServlet +" path: "+path+" service type: "+service.getServiceType());
        try {
            ...
            if (serviceType==ServiceType.ROOT) {
                ...
                if (defaultServlet) {
                    ...
                    System.out.println("AU FILTER DEFAULT SERVLET Inside Forward REquest defaultServlet true condition path= "+path); 
                }
                ...
                System.out.println("AU FILTER ==== AUDEBUG PATH ==== "+path);
            }
        	...
        	if(req!=null){
        		System.out.println("AU FILTER AU_FILTER.forwardRequest exception req.getServerName(): " + req.getServerName());
        		System.out.println("AU FILTER AU_FILTER.forwardRequest exception domain: " + domain);
        		System.out.println("AU FILTER AU_FILTER.forwardRequest exception service: " + service);        		
        		System.out.println("AU FILTER AU_FILTER.forwardRequest exception user: " + user);        		
        		System.out.println("AU FILTER AU_FILTER.forwardRequest exception lang: " + lang);        		
        		System.out.println("AU FILTER AU_FILTER.forwardRequest exception serviceType: " + serviceType); 
        		System.out.println("AU FILTER AU_FILTER.forwardRequest exception path: " + path); 
        	}
        	
        	System.out.println("AU FILTER AU_Filter UmeCore.forwardRequest exception : " + e);
                e.printStackTrace();
