 ...
 * @author madan
 */
@WebServlet(name = "IEConfirm", urlPatterns = {"/IEConfirm"})
public class IEConfirm extends HttpServlet {
    ...
    NetworkMapping networkMapping;
        ...
        private final Logger logger = LogManager.getLogger(IEConfirm.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //logger.info("ieconfirm "+"ProcessREQEST is called upon ");

        HttpSession session = request.getSession();
        //logger.info("ieconfirm "+"session id is  "+session.getId());
        ServletContext application = request.getServletContext(); //.setAttribute(null, request);
        //logger.info("ieconfirm "+"servletContext is  "+application.getContextPath());
	...
	userClubDetails=UmeTempCmsCache.umeClubDetailsMap.get(club.getUnique());
        ...
        System.out.println("IEMO subscription network original " + networkname.toLowerCase());
            try{
                networkname=mobilenetwork.getMobileNetwork("IE",networkname);
            }catch(Exception e){networkname="unknown";}
        System.out.println("IEMO subscription network after " + networkname.toLowerCase());
        ...
        if(session.getAttribute("vodafoneidentified")==null && session.getAttribute("msisdnidentified")==null){
         campaigndao.log("ieconfirmweb3", confirmlanding, msisdn,msisdn, handset,domain, campaignId, club.getUnique(), "MANUAL", 0, request,response,networkname.toLowerCase(),"","","",pubId);   
        }
        ...
        //==========END IF USER EXISTS OR NOT ============================
              ...
              if(sendTeaser){
                  ...
                  if(userClubDetails.getBillingType().equalsIgnoreCase(("adhoc"))){
                    ...
                    logger.info("IEMO subscription  " + clubUser.toString() + " Create Billing Plan is " + createbillPlan);
                }
                ...
                System.out.println("IEMO subscription  " +clubUser.getParsedMobile()+" value of createbillPlan is "+createbillPlan);
                  if (billingplan == null || createbillPlan) {
                      System.out.println("IEMO subscription network adhoc " +clubUser.getParsedMobile()+" NETWORKNAME before if " + networkname.toLowerCase());
                      ...
                      if(clubUser.getNetworkCode().isEmpty() || clubUser.getNetworkCode().equalsIgnoreCase("unknown")){
                          System.out.println("IEMO subscription network adhoc " +clubUser.getParsedMobile()+" NETWORKNAME before looking up " + networkname);
                          ...
                          System.out.println("IEMO subscription network adhoc " +clubUser.getParsedMobile()+" NETWORKNAME after looking up " + (networkname==null?"null":networkname));
                          if(networkname==null || networkname.isEmpty()) networkname=networkMapping.getIeNetworkMap().get(mncCode.substring(1));
                          clubUser.setNetworkCode(networkname);
                          } else{
                              System.out.println("IEMO subscription network adhoc mncCode is blank for "+clubUser.getParsedMobile()+" at "+new Date());
                          }
                          
                      }                      
                      
                      System.out.println("IEMO subscription network adhoc " +clubUser.getParsedMobile()+" NETWORKNAME BEFORE SMSNUMBER LOOK UP  " + networkname);
                      ...
                      System.out.println("IEMO subscription network adhoc " +clubUser.getParsedMobile()+" NETWORKNAME DEFINITIVE " + networkname);
