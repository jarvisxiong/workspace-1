 ...
 * @author madan
 */
@WebServlet(name = "GRConfirm", urlPatterns = {"/GRConfirm"})
public class GRConfirm extends HttpServlet {
    ...
    NetworkMapping networkMapping;
        ...
        private final Logger logger = LogManager.getLogger(GRConfirm.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        logger.info("grconfirm "+"ProcessREQEST is called upon ");

        HttpSession session = request.getSession();
        logger.info("grconfirm "+"session id is  "+session.getId());
        ServletContext application = request.getServletContext(); //.setAttribute(null, request);
        logger.info("grconfirm "+"servletContext is  "+application.getContextPath());
	...
	userClubDetails=UmeTempCmsCache.umeClubDetailsMap.get(club.getUnique());
        ...
        System.out.println("grconfirm subscription network original " + networkname.toLowerCase());
            try{
                networkname=mobilenetwork.getMobileNetwork("GR",networkname);
            }catch(Exception e){networkname="unknown";}
        System.out.println("grconfirm subscription network after " + networkname.toLowerCase());
        if(msisdn.startsWith("0")){
           msisdn="30"+msisdn.substring(1);
        }
        ...
        if(session.getAttribute("vodafoneidentified")==null && session.getAttribute("msisdnidentified")==null){
         campaigndao.log("grconfirmweb3", confirmlanding, msisdn,msisdn, handset,domain, campaignId, club.getUnique(), "MANUAL", 0, request,response,networkname.toLowerCase(),"","","",pubId);   
        }
	...
	if (!userUnique.equals("")) user = umeuserdao.getUser(msisdn);
        ...
        if ((clubUser!=null && clubUser.getActive()!=1) ||(clubUser==null)) {
            ...
            if(networkname.toLowerCase().contains("wind")){
                networkname="WIND";                         
            }
            else if(networkname.toLowerCase().contains("vodafone")){
                networkname="VODAFONEGR";                     
            }
            else if(networkname.toLowerCase().contains("cosmo")){
                networkname="COSMOSTE";                         
            }
            //This can be used for HLR-LookUp. 
            // String mncCode=getNetwork(msisdn);
        ...
        //==========END IF USER EXISTS OR NOT ============================
              ...
              if(sendTeaser){
                  ...
                  if(userClubDetails.getBillingType().equalsIgnoreCase(("adhoc"))){
                    ...
                    logger.info("grconfirm subscription  " + clubUser.toString() + " Create Billing Plan is " + createbillPlan);
                }
                ...
                if (subsresponse.equals("SUBSCRIPTION RECORD ALREADY EXISTS")) {
                    ...
                    System.out.println("grconfirm subscription  " + clubUser.toString() + " createbillPlan IS " + createbillPlan);
                }
                ...
                System.out.println("grconfirm subscription  " +clubUser.getParsedMobile()+" value of createbillPlan is "+createbillPlan);
                  if (billingplan == null || createbillPlan) {
                      System.out.println("grconfirm subscription network adhoc " +clubUser.getParsedMobile()+" NETWORKNAME before if " + networkname.toLowerCase());
                      if(networkname.toLowerCase().contains("vodafone")){
                          networkname="VODAFONE9IE";
                          clubUser.setNetworkCode(networkname);
                      }
                      else if(networkname.toLowerCase().contains("wind")){
                          networkname="WIND";
                          clubUser.setNetworkCode(networkname);
                      }
                      else if(networkname.toLowerCase().contains("vodafone")){
                      ...
                      if(clubUser.getNetworkCode().isEmpty() || clubUser.getNetworkCode().equalsIgnoreCase("unknown")){
                          System.out.println("grconfirm subscription network adhoc " +clubUser.getParsedMobile()+" NETWORKNAME before looking up " + networkname);
                          ...
                          System.out.println("grconfirm subscription network adhoc " +clubUser.getParsedMobile()+" NETWORKNAME after looking up " + (networkname==null?"null":networkname));
                          if(networkname==null || networkname.isEmpty()) networkname=networkMapping.getIeNetworkMap().get(mncCode.substring(1));
                          clubUser.setNetworkCode(networkname);
                          } else{
                              System.out.println("grconfirm subscription network adhoc mncCode is blank for "+clubUser.getParsedMobile()+" at "+new Date());
                          }
                          
                      }                      
                      
                      System.out.println("grconfirm subscription network adhoc " +clubUser.getParsedMobile()+" NETWORKNAME BEFORE SMSNUMBER LOOK UP  " + networkname);
                      ...
                      System.out.println("grconfirm subscription network adhoc " +clubUser.getParsedMobile()+" NETWORKNAME DEFINITIVE " + networkname);
        ...
        if(campaignId!=null && !campaignId.equalsIgnoreCase("")){
         ...
         String cpalogQuery="UPDATE cpavisitlog set aParsedMobile='"+msisdn+"',isSubscribed='1',aSubscribed='2011-01-01' WHERE aCampaignId='"+cmpg.getUnique()+"' "
            ...
            if(sendTeaser) {
                /**
                 * Greece Subscription HTTP URL Request for Pending !! 
                 */
