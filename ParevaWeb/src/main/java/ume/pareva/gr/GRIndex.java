 ...
 * @author madan
 */
@WebServlet(name = "GRIndex", urlPatterns = {"/GRIndex"})
public class GRIndex extends HttpServlet {
	private final Logger logger = LogManager.getLogger(GRIndex.class.getName());
	 ...
	 */
	 ...
	 protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		 ...
		 logger.info("grindex "+"ProcessREQEST is called upon ");

		 HttpSession session = request.getSession();
		 logger.info("grindex "+"session id is  "+session.getId());
		 ServletContext application = request.getServletContext(); //.setAttribute(null, request);
		 logger.info("grindex "+"servletContext is  "+application.getContextPath());
		 ...
		 logger.info("grindex "+" after SDC request ");
		 ...
		 System.out.println("grindex "+" FileName is  "+fileName);
		 ...
		 logger.info("grindex "+"contentUrl is  "+cloudfronturl);
		 ...
		 if (!enMsisdn.equals("")) {
			 System.out.println("grindex msisdn the mid parameter is "+enMsisdn);
			 context.put("mid", enMsisdn);
			 String deMsisdn = MiscCr.decrypt(enMsisdn);
			 if (!deMsisdn.equals("")) {
				 if (deMsisdn.startsWith("0")) {
					 deMsisdn = "30" + deMsisdn.substring(1);
					 System.out.println("iemsisdn the deMsisdn value of mid "+enMsisdn+"  is "+deMsisdn);
				 }
				 msisdn = deMsisdn;
				 System.out.println("grindex msisdn  finally the value of msisdn is "+msisdn+"  of mid "+enMsisdn);
		 ...
		 try {
			 ...
			 System.out.println("GR INDEX JSP LOGGING TO CAMPAIGN LOG ");
			 if (!aReq.get("cid").equalsIgnoreCase("") && session.getAttribute("bilogged")==null) {
				 campaigndao.log("grindex", landingPage, msisdn, msisdn, handset, domain, campaignId, clubUnique, "INDEX", 0, request, response, networkname,"","","",pubId);

				 if(msisdn!=null && !msisdn.equalsIgnoreCase(""))
					 campaigndao.log("grindex", landingPage, msisdn, msisdn, handset, domain, campaignId, clubUnique, "IDENTIFGRD", 0, request, response, networkname,"","","",pubId);
			 ...
			 System.out.println("GR Index Error for campaignlog " + e);
			 logger.error("GR INDEX ERROR FOR CAMPAIGN LOG {}", e.getMessage());
			 logger.error("GR INDEX ERROR FOR CAMPAIGN LOG ", e);
		 }
			 ...
			 if(userclubdetails.getBillingType().equalsIgnoreCase("subscription")){
				 ...
				 if(userclubdetails.getServiceType().equalsIgnoreCase("content")) {
					 ...
					 try {
						 ...
						 logger.error("grindex wapxgreece  EXCEPTION {}", e.getMessage());
						 logger.error("grindex wapxgreece EXCEPTION ", e);
					 } 
					 ...
					 if(user!=null){
						 ...
						 try {
							 ...
							 logger.error("grindex EXCEPTION {}", e.getMessage());
							 logger.error("grindex EXCEPTION ", e);
						 }
				 ...
				 System.out.println("grindex realgirls  adhoc service condition ");
				 if(user!=null){
					 request.setAttribute("user", user);
					 SdcMobileClubUser clubUser=umemobileclubuserdao.getClubUserByMsisdn(user.getParsedMobile(),club.getUnique());
					 System.out.println("grindex realgirls user is not null ");
					 if(clubUser!=null && clubUser.getActive()==1){
						 System.out.println("grindex realgirls clubUser is active "+clubUser.getParsedMobile());
						 ...
						 if(billingplan!=null && billingplan.getActiveForBilling()<0 && billingplan.getExternalId().equalsIgnoreCase("adhocpending")){
							 ...
							 campaigndao.log("gradhoc", clubUser.getLandingpage(), clubUser.getParsedMobile(),clubUser.getParsedMobile(), handset,domain, clubUser.getCampaign(),clubUser.getClubUnique(), "SUBSCRIBED", 0, request,response,clubUser.getNetworkCode().toLowerCase(),"","","",pubId);
							 try {
							 ...
							 try {
								...
								return;
								 ...
								 logger.error("grindex WAPXIRE EXCEPTION {}", e.getMessage());
								 logger.error("grindex WAPXIRE EXCEPTION ", e);
							 } 
						 } 
					 } //clubuser!=null
				 }  //user!=null 
