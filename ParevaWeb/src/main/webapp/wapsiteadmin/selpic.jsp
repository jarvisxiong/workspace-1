<%@ include file="/WEB-INF/jspf/wapsiteadmin/selpic.jspf" %>
<html>
<head>
...
<td valign="top" align="left">
    ...
    <tr>
            ...
            <a href="<%=srcPage%>.jsp?cmd=<%=cmd%>&srvc=<%=srvc%>&fp=<%=fp%>">Back</a>
            </td>
    </tr>	
    </table>
...
<%      } %>		
        ...
        <td align="center" valign="bottom">
          <a href="<%=srcPage%>.jsp?updpic=<%=Misc.hex8Code(fName)%>&cmd=<%=cmd%>&srvc=<%=srvc%>&pic=<%=pic%>&restype=<%=restype%>"><img src="/images/javagames/promos/source/<%=fName%>" border="0"></a>
          <br>
          <a href="<%=srcPage%>.jsp?updpic=<%=Misc.hex8Code(fName)%>&cmd=<%=cmd%>&srvc=<%=srvc%>&pic=<%=pic%>&restype=<%=restype%>"><span class="grey_10"><%=alias%></span></a>
          <br>
          <a href="javascript:win('del.jsp?picid=<%=Misc.hex8Code(fName)%>&cmd=<%=cmd%>&srvc=<%=srvc%>&pic=<%=pic%>&fp=<%=fp%>')">Delete</a>
        </td>
<%		
		cc++;
