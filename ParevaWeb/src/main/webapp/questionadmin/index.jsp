            ...
            <div class="portlet-body">
                <div class="table-toolbar">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="btn-group">
                                <button id="sample_editable_1_new" class="btn sbold green js_loadURL" 
                                data-url="../questionadmin/add.jsp"> 
                                    Create New Question
                                    <i class="fa fa-plus"></i>
                                </button>
                                <!-- <button id="sample_editable_1_new" class="btn sbold green js_loadURL" data-url="../questionadmin/add.jsp"> 
                                 <a href="../questionadmin/add.jsp">Create New Question</a>
                                    <i class="fa fa-plus"></i>-->   
                                <!-- </button> -->
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="btn-group pull-right">
                                <Select class="btn green  btn-outline dropdown-toggle" data-toggle="dropdown" name="question_pack_id">
                                    <option value="">Select Question Pack</option>
                                    <c:forEach items="${questionPackList}" var="questionPack">
                                  		<option value="${questionPack.aUnique}">${questionPack.aName}	
	                    ...
	                    <tbody> 
		       				<c:forEach items="${questionList}" var="question">
						        <tr class="gradeX" role="row"> 
									...
									<td>
									<fmt:formatDate value="${question.aCreated}" pattern="yyyy-MM-dd HH:mm:ss"/>
										<%-- <c:out value="${questionPack.aCreated}" /> --%>
									</td>
									<td>
                                        <a class="btn btn-xs green js_loadURL" data-url="../questionadmin/edit.jsp?action=Update&questionId=${question.questionId}">
                                            <i class="fa fa-cogs"></i> 
                                            Edit
                                        </a>
										<%-- <a class="btn btn-xs green js_loadURL" data-url="../questionpack/edit.jsp?action=Update&unique=${questionPack.aUnique}">
                                            <i class="fa fa-cogs"></i> 
                                            <a href="../questionadmin/edit.jsp?action=Update&questionId=${question.questionId}">Edit</a>--%> 
                                        <!-- </a> -->
									</td>
						        </tr>
						    </c:forEach>
...
$( document ).ready(function() {
    ...
    $(".nav-item a, .js_loadURL").off('click').on("click", function() {
        ...
        if (url.indexOf("wapsiteadmin") >= 0) {
            ...
            || url.indexOf("question") >= 0 || url.indexOf("competition") >= 0 ) {
            var loadJQ = true;
            stop = 0;
        } else {
            var loadJQ = false;
