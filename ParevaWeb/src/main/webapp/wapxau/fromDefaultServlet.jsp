...
<%
try {
System.out.println("AUdebug FromDEfaultServlet called upon ");
//***************************************************************************************************
...
AUItemTicket auitemtickets=null;
 try{
	  ...
	  umemobileclubuserdao=(UmeMobileClubUserDao)ac.getBean("umemobileclubuserdao");
           auitemtickets=(AUItemTicket) ac.getBean("auitemticket");
...
if (contentItem!=null) {
    ...
    itemTicket = auitemtickets.get(contentItem, client.getUnique(), msisdn, logUnique, cType, iType, iLang, iParam, dmn.getDefaultUrl());
	System.out.println("AUdebug ITEM TICKET = "+itemTicket);
...
//        logger.drmSupport = "forwardLock: " + handset.supportsForwardLock();
        ...
        try { logger.endUserPrice = ((Double) UmeTempCmsCache.domainPriceGroups.get(domain + "_" + logger.priceGroup + "_price")).doubleValue();
            } catch (NullPointerException ex) { logger.endUserPrice=0;//System.out.println("endUserPrice " + ex); 
...
if (itemFound) {
System.out.println("AUdebug Exception ticketfound : "+ itemTicket);
...
return;
} catch (Exception e) { System.out.println("AUdebug fromDefaultServlet:  " + e); e.printStackTrace();}
%>