        ...
        if (campaignId != null && campaignId.trim().length() > 0) {
            ...
            if (!campaignId.equals("")) {
                ...
                if (campaignId.equals("2409942441KDS")) {
                    System.out.println("ItalyCPA " + campaignId);
                }
            }
            if (cmpg != null) {
                campaignsrc = cmpg.getSrc();
                if (cmpg.getUnique().equals("2409942441KDS")) {
                    System.out.println("ItalyCPA " + cmpg.getUnique() + " " + cmpg.getSrc());
                }
            }
            try {
                if (cmpg != null) {
...
//                    if(domain.equalsIgnoreCase("7609812504441llun") && myisp.toLowerCase().contains("wind"))
//                    {
//			System.out.println("ITsoloredirect happening on "+domain+" myisp is "+myisp+"for cid "+cmpg.getUnique()+"- "+cmpg.getSrc());
//                        response.sendRedirect("http://videocasalinghi.realgirls.mobi/?cid="+cmpg.getUnique());
//                        return;
//                    }
                }
            } catch (Exception e) {
        ...
        if (club != null && club.getRegion().equals("IT") && user == null && isIdentified) //if(club!=null && club.getRegion().equals("IT") && user==null)
        {
            System.out.println("ITCONFIRM User is NULL here ======= ");
%>
...
<%
        ...
        try {
            System.out.println("IT INDEX ====  GOING TO INDEX_MAIN.JSP NOW ====  " + "/" + System.getProperty("dir_" + domain + "_pub") + "/index_main.jsp");
