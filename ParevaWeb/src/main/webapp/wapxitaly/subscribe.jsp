...
//SubscriptionApiPort subscriptionApi = new SubscriptionApiServiceLocator().getSubscriptionApi31(new URL(SUBSCRIBE_REQUEST_URL));
//((SubscriptionApiBindingStub) subscriptionApi).setTimeout(50000);

SubscriptionApiPort subscriptionApi = new SubscriptionApiServiceLocator()
...
//String DELIVER_TARGET = "/servlet/deliversubscribe";
...
//	System.out.println(crlf + "IPX Subscription Delivery URL: " + aDeliveryURL);
	...
	try {
		// if operator is vodafone make the vodafone request
		if( (operator.equals("vodafone") || operator.equals("wind") || operator.equals("tim") || operator.equals("three"))  && webpin.equals("")){
		//if( (operator.equals("vodafone") || operator.equals("wind") || operator.equals("tim"))){
//			System.out.println("InsideVodafone");

			CreateSubscriptionSessionResponse aCreateSessionResponse = 
				iFrameOperatorSubscriptionRequest(transactionId, msisdn, operator, ServiceMetaData, clubUnique, clubIPXUserName, clubIPXPassword, subscriptionApi, aDeliveryURL, club);		

//			System.out.println("InsideVodafone aCreateSessionResponse");
			
                        dd.append(crlf + "IPX Subscription Session: " + MiscDate.now24sql() + "--" + msisdn + "--" + operator + "--" + transactionId + "--" +ServiceMetaData + "--" + aDeliveryURL);
...
//						+ aCreateSessionResponse.getResponseMessage() + ")");
			...
			dd.append(crlf + "Session sessionID: " + aCreateSessionResponse.getSessionId() + "--" + aCreateSessionResponse.getRedirectURL());
//			dd.append(crlf + "IPX Subscription Session redirect URL: " + aCreateSessionResponse.getRedirectURL());		
			...
			CreateSubscriptionResponse aResponse = 
				threeOperatorSubscriptionRequest(transactionId, msisdn, operator, ServiceMetaData, clubUnique, clubIPXUserName, clubIPXPassword, subscriptionApi, club);
			...
			//TODO redirect THREE
			if(operator.toLowerCase().equals("three")){
				//TODO check with clubUnique or service
				System.out.println("IPX confirm_target: " + CONFIRM_TARGET);
				String threeRedirectLink = "";
				if(clubIPXUserName.equalsIgnoreCase("umesolo18-it"))
				{
					threeRedirectLink = "http://sdc.tre.it/p-sdc/m-site/ActiveHub/ServiceConfirmActivation_H?sid=1f537623X2v000c&pid=10014&lp=http%3A%2F%2Fsolo18a.realgirls.mobi" + CONFIRM_TARGET;
				}else{
					threeRedirectLink = "http://sdc.tre.it/p-sdc/m-site/ActiveHub/ServiceConfirmActivation_H?sid=6748c1edOEF0032&pid=10014&lp=http%3A%2F%2Fvideocasalinghi.realgirls.mobi" + CONFIRM_TARGET;
				}
                                System.out.println("IPX threeRedirectLink " + threeRedirectLink);
				response.sendRedirect(threeRedirectLink);
				return;
			}
			System.out.println(dd.toString());		
		...
		//dd.append(crlf + "IPX Subscription Session Got RemoteException when processing request" + e.getMessage());
		//WriteFile(debugPath, debugFile, dd, crlf);
