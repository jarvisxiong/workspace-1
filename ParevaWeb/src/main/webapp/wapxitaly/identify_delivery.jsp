        ...
        if (aSessionId != null) {
            if (checkIfAuthorized(aSessionId, clubIPXUserName, clubIPXPassword, myApi, request, campaignId)) {
                ...
                if(msisdn!=null && !msisdn.isEmpty()){
                    ...
                    campaigndao.log("confirm", landingPage, msisdn, msisdn, handset, domain, campaignId, clubUnique, "IDENTIFIED", 1, request, response, operator.toLowerCase(),"","","",pubId);                    
...
//                if (operator.toLowerCase().equals("vodafone") || operator.toLowerCase().equals("wind") || operator.toLowerCase().equals("tim") || operator.toLowerCase().equals("three")) {
                    try {
                        ...
                        //application.getRequestDispatcher("/" + System.getProperty("dir_" + domain + "_pub") + "/subscribe.jsp" + "?cid=" + campaignId).forward(request, response);
                        return;
                    } catch (Exception ee) {
