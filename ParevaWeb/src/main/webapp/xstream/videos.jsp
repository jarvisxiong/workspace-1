...
String visiteddomain="i.x-stream1.co.uk";//httprequest.get("dom");
//Transaction trans=dbsession.beginTransaction();

String srvc="";
String cat = httprequest.get("cat");
int number_of_elements = Integer.parseInt(httprequest.get("noe"));
...
java.util.List categoryList=(java.util.List)videoParameter.get("categoryList");
//java.util.List categoryList=(java.util.List) session.getAttribute("categoryList");

HashMap categoryMap=videolistuk.getVideosByCategory(index, cat, "", domain, handset, number_of_elements, lang, pfitagHeaders, Refrence_id, dmn, imiMsisdn, imiAddCode_, imi_price);

int listSize=Integer.parseInt(categoryMap.get("listSize").toString());
java.util.List videos=(java.util.List)categoryMap.get("videos");

//java.util.List categoryVideos=videolistuk.getVideosByCategory(index, cat, "", domain, handset, number_of_elements, lang, pfitagHeaders, Refrence_id, dmn, imiMsisdn, imiAddCode_, imi_price);

int maxCount = number_of_elements;
//int pageCount = (list.size()-index)/maxCount;
int pageCount = listSize/maxCount;
//if((list.size()-index)%maxCount!=0)
if((listSize-index)%maxCount!=0)	
	pageCount=pageCount+1;
int curPage = (index/maxCount)+1;

//int totalPage=list.size()/maxCount;
int totalPage=listSize/maxCount;
//if(list.size()%maxCount!=0)
if(listSize%maxCount!=0)
...
int value = img.lastIndexOf('/');
    ...
    String data_reference = Refrence_id;
     String data_content_location = "http://" + dmn.getDefaultUrl()
      + "/videodetail.jsp?unq=" + item.getUnique()+"&srvc=" + srvc+"&data-reference="+data_reference+"&cntid=" + data_content_id+"&msisdn=" + imiMsisdn+"&s=" + imiAddCode_;
...
context.put("cat",cat);
context.put("noe",number_of_elements);
context.put("action","videos.jsp");
