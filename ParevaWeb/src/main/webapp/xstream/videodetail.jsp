...
ItemImage image = item.getItemImage("mobthumb", 0, 0);
...
java.util.List video_category_list=(java.util.List)application.getAttribute("promo_hot_video_list");
System.out.println("xstreamukcategory "+video_category_list.size());
...
HashMap relatedVideoMap=videolistuk.getVideosByCategory(pagination_index, cat, unique, domain, handset, number_of_elements, lang, pfitagHeaders, Refrence_id, dmn, imiMsisdn, imiAddCode_, imi_price);

int listSize=Integer.parseInt(relatedVideoMap.get("listSize").toString());
java.util.List videos=(java.util.List)relatedVideoMap.get("videos");

int maxCount = number_of_elements;

int pageCount = listSize/maxCount;

if((listSize-pagination_index)%maxCount!=0)	
...
context.put("index",pagination_index);
...
engine.getTemplate("purchased").evaluate(writer, context);
